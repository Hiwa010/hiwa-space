...
export const HeroParallax: React.FC<HeroParallaxProps> = ({
  ...
  // Check if it's mobile device to avoid heavy processing
  ...
  const drawFrame = useCallback((frameIdx: number) => {
    ...
    // On mobile, lock it to frame 0 (or a single static frame) to guarantee zero lag
    const targetIdx = isMobile ? 0 : Math.max(0, Math.min(TOTAL_WEBP_FRAMES - 1, Math.floor(frameIdx)));
  ...
  useEffect(() => {
    ...
    // Load extra frames only if not on mobile for performance
    if (!isMobile) {
      const loadFramesInBackground = () => {
        for (let i = 1; i < TOTAL_WEBP_FRAMES; i += 10) {
          const img = new Image();
  ...
  // Scroll listener (disabled on mobile for smooth performance)
  useEffect(() => {
    if (isMobile) return;
  ...
  return (
    <div
      ...
      className="relative w-full lg:h-[320vh] h-auto bg-[#070709]"
    >
      <div className="relative lg:sticky top-0 h-screen w-full overflow-hidden flex flex-col justify-between">
              ...
              <div className="flex items-center gap-2.5">
                {!isMobile && (
                  <button
                    ...
                    className="flex items-center gap-1.5 rounded-full border border-white/10 bg-black/60 backdrop-blur-md px-3 py-1.5 text-xs font-medium text-zinc-300 hover:text-white transition-all"
                  >
                    {isPlayingAuto ? <Pause className="h-3.5 w-3.5 text-orange-400" /> : <Play className="h-3.5 w-3.5" style={{ color: activeTheme.accentColor }} />}
